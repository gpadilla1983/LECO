...
$('#table-estado_caso_legal').DataTable({
    responsive: true
});
...
function btn_consultarClick() {
  ...
  $.ajax({
    ...
    url: base_url + "catEstadoCasoLegal/ObtenerEstadoCasoLegal",
    data: ArrayPOST,
...
$("#btn_agregar_estado_caso_legal").bind("click", async (e) => {
  ...
  var html = await $.ajax({
    ...
    url: base_url + "ModalEstadoCasoLegal/0",
  });
  ...
  $("#modalDatosEstadoCasoLegal").modal("show");
});

async function EditarEstadoCasoLegal(id_estado_caso_legal) {
  if ($("#ajax-content").length) {
    $("#ajax-content").remove();
  }
  ...
  var html = await $.ajax({
    ...
    url: base_url + "ModalEstadoCasoLegal/" + id_estado_caso_legal,
  });
  ...
  $.ajax({
    type: "GET",
    async: true,
    url: base_url + "catEstadoCasoLegal/getEstadoCasoLegal/" + id_estado_caso_legal,
    success: function (respuesta) {
      ...
      var datos = JSON.parse(respuesta);
        ...
        $("#ddl_estatus_estado_caso_legal").val(datos.id_estado_logico);
        $("#div_estatus").show();
        $("#modalDatosEstadoCasoLegal").modal("show");
    },
  });
...
function AgregarActualizarEstadoCasoLegal(id_estado_caso_legal) {
  ...
  $.ajax({
    ...
    url: base_url + "catEstadoCasoLegal/AgregarActualizarEstadoCasoLegal/" + id_estado_caso_legal,
    data: $("#formModalEstadoCasoLegal").serialize(),
    success: function (respuesta) {
      $("body").dynamicSpinnerDestroy({});
      $("#modalDatosEstadoCasoLegal").modal("hide");  
      if (respuesta == 1) {
        
        swal("Estado Caso Legal agregado o actualizado con éxito", "", "success");
        btn_consultarClick();
      } else {
        if (respuesta == 0) {
