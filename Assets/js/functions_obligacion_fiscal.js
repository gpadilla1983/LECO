...
$('#table-obligacion_fiscal').DataTable({
    responsive: true
});
...
function btn_consultarClick() {
  ...
  $.ajax({
    ...
    url: base_url + "obligacionFiscal/ObtenerObligacionFisc",
    data: ArrayPOST,
...
$("#btn_agregar_obligacion_fiscal").bind("click", async (e) => {
  ...
  var html = await $.ajax({
    ...
    url: base_url + "ModalObligacionFiscal/0",
  });
  ...
  $("#modalDatosObligacionFisc").modal("show");
});

async function EditarObligacionFisc(id_obligacion_fiscal) {
  if ($("#ajax-content").length) {
    $("#ajax-content").remove();
  }
  ...
  var html = await $.ajax({
    ...
    url: base_url + "ModalObligacionFiscal/" + id_obligacion_fiscal,
  });
  ...
  $.ajax({
    type: "GET",
    async: true,
    url: base_url + "obligacionFiscal/getObligacionFisc/" + id_obligacion_fiscal,
    success: function (respuesta) {
      ...
      var datos = JSON.parse(respuesta);
        ...
        $("#ddl_estatus_obligacion_fiscal").val(datos.id_estado_logico);
        $("#div_estatus").show();
        $("#modalDatosObligacionFisc").modal("show");
    },
  });
...
function AgregarActualizarObligacionFisc(id_obligacion_fiscal) {
  $.ajax({
    ...
    url: base_url + "obligacionFiscal/AgregarActualizarObligacionFisc/" + id_obligacion_fiscal,
    data: $("#formModalObligacionFisc").serialize(),
    success: function (respuesta) {
      $("body").dynamicSpinnerDestroy({});
      $("#modalDatosObligacionFisc").modal("hide");  
      if (respuesta == 1) {
        
        swal("Obligación Fiscal agregada o actualizada con éxito", "", "success");
        btn_consultarClick();
      } else {
        if (respuesta == 0) {
