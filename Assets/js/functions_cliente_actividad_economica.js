...
$('#table-cliente_actividad_economica').DataTable({
    responsive: true
});
...
function btn_consultarClick() {
  var ArrayPOST = {
    ...
    ddl_actividad_economica: $("#ddl_actividad_economica").val(),
  };
  ...
  $.ajax({
    ...
    url: base_url + "relClienteActividadEconomica/ObtenerClienteActividadEconomica",
    data: ArrayPOST,
      ...
      if (respuesta.length > 0) {
        $("#table-cliente_actividad_economica").DataTable({
          ...
          columns: [
            ...
            { data: "datos_actividad_economica", className: "text-center" },
...
$("#btn_agregar_cliente_actividad_economica").bind("click", async (e) => {
  ...
  var html = await $.ajax({
    ...
    url: base_url + "ModalClienteActividadEconomica/0",
  });
  ...
  $("#modalDatosClienteActividadEconomica").modal("show");
});

async function EditarClienteActividadEconomica(id_cliente_actividad_economica) {
  if ($("#ajax-content").length) {
    $("#ajax-content").remove();
  }
 ...
 //debugger;
  ...
  var html = await $.ajax({
    ...
    url: base_url + "ModalClienteActividadEconomica/" + id_cliente_actividad_economica,
  });
  ...
  $.ajax({
    type: "GET",
    async: true,
    url: base_url + "relClienteActividadEconomica/getClienteActividadEconomica/" + id_cliente_actividad_economica,
    success: function (respuesta) {
      ...
      var datos = JSON.parse(respuesta);
        ...
        //console.log(datos.id_actividad_economica);
        //console.log(datos.id_estado_logico);
        $("#tbx_tipo_cliente").val(datos.tipo_cliente);
        $("#tbx_cliente").val(datos.datos_cliente);
        $("#tbx_actividad_economica").val(datos.datos_actividad_economica);
        $("#ddl_estatus_cliente_actividad_economica").val(datos.id_estado_logico);
        $("#div_estatus").show();
        $("#modalDatosClienteActividadEconomica").modal("show");
    },
  });
...
function AgregarActualizarClienteActividadEconomica(id_cliente_actividad_economica) {
  ...
  $.ajax({
    ...
    url: base_url + "relClienteActividadEconomica/AgregarActualizarClienteActividadEconomica/" + id_cliente_actividad_economica,
    data: $("#formModalClienteActividadEconomica").serialize(),
    success: function (respuesta) {
      $("body").dynamicSpinnerDestroy({});
      $("#modalDatosClienteActividadEconomica").modal("hide");  
      if (respuesta == 1) {
        swal("Relación Cliente - Actividad Económica actualizada con éxito", "", "success");
        btn_consultarClick();
      } else {
        if (respuesta == 0) {
