...
$("#btn_agregar_pais").bind("click", async (e) => {
...
async function EditarPais(id_pais) {
  ...
  var html = await $.ajax({
    ...
    url: base_url + "ModalPais/" + id_pais,
  });
  ...
  $.ajax({
    type: "GET",
    async: true,
    url: base_url + "pais/getPais/" + id_pais,
    success: function (respuesta) {
      ...
      var datos = JSON.parse(respuesta);
        ...
        $("#ddl_estatus_pais").val(datos.id_estado_logico);
        $("#div_estatus").show();
        $("#modalDatosPais").modal("show");
    },
  });
...
function AgregarActualizarPais(id_pais) {
  ...
  $.ajax({
    ...
    url: base_url + "pais/AgregarActualizarPais/" + id_pais,
    data: $("#formModalPais").serialize(),
    success: function (respuesta) {
      $("body").dynamicSpinnerDestroy({});
      $("#modalDatosPais").modal("hide");  
      if (respuesta == 1) {
        
        swal("País agregado o actualizado con éxito", "", "success");
        btn_consultarClick();
      } else {
        if (respuesta == 0) {
