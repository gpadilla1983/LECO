...
$('#table-regimen_fiscal').DataTable({
    responsive: true
});
...
function btn_consultarClick() {
  ...
  $.ajax({
    ...
    url: base_url + "regimenfiscal/ObtenerRegimenFiscal",
    data: ArrayPOST,
...
$("#btn_agregar_regimen_fiscal").bind("click", async (e) => {
  ...
  var html = await $.ajax({
    ...
    url: base_url + "ModalRegimenFiscal/0",
  });
  ...
  $("#modalDatosRegimenFiscal").modal("show");
});

async function EditarRegimenFiscal(id_regimen_fiscal) {
  if ($("#ajax-content").length) {
    $("#ajax-content").remove();
  }
  ...
  var html = await $.ajax({
    ...
    url: base_url + "ModalRegimenFiscal/" + id_regimen_fiscal,
  });
  ...
  $.ajax({
    type: "GET",
    async: true,
    url: base_url + "regimenfiscal/getRegimenFiscal/" + id_regimen_fiscal,
    success: function (respuesta) {
      ...
      var datos = JSON.parse(respuesta);
        ...
        $("#ddl_estatus_regimen_fiscal").val(datos.id_estado_logico);
        $("#div_estatus").show();
        $("#modalDatosRegimenFiscal").modal("show");
    },
  });
...
function AgregarActualizarRegimenFiscal(id_regimen_fiscal) {
  ...
  $.ajax({
    ...
    url: base_url + "regimenfiscal/AgregarActualizarRegimenFiscal/" + id_regimen_fiscal,
    data: $("#formModalRegimenFiscal").serialize(),
    success: function (respuesta) {
      $("body").dynamicSpinnerDestroy({});
      $("#modalDatosRegimenFiscal").modal("hide");  
      if (respuesta == 1) {
        
        swal("Régimen Fiscal agregado o actualizado con éxito", "", "success");
        btn_consultarClick();
      } else {
        if (respuesta == 0) {
